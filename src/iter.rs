...
trait ZipsIntoOne {
    ...
    fn zip_into_one(
        &self,
        iters: Vec<Box<dyn Iterator<Item = Self::Item>>>,
        zprex: &str,
...
impl<I> ZipsIntoOne for dyn Iterator<Item = I> {
    ...
    fn zip_into_one(
        ...
        let mut total_iters: Vec<Box<dyn Iterator<Item = Self::Item>>> = vec![Box::from(self)];
        total_iters.append(&mut iters);
...
trait AllZipsIntoOne {
    type Item;
    fn zip_into_one(&mut self, zprex: &str) -> Result<Zipper<Self::Item>>;
}
